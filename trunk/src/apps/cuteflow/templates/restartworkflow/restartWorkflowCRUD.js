cf.restartWorkflowCRUD = function(){return {
	...
	createSavePanel: function () {
		...
		for(var a=0;a<userPanel.items.length;a++) {
			...
			for(var b=0;b<usergrid.store.getCount();b++) { // save the users in the slots
				...
				cf.restartWorkflowCRUD.theSavePanel.add(hiddenfield);	
			}
	...
	addFile: function (slotcounter,fieldid, field, fieldcounter) {
		...
		cf.restartWorkflowCRUD.theSavePanel.add(field);
	},
	...
	doSubmit: function () {
		cf.restartWorkflowCRUD.theLoadingMask.hide();
	...
	initSave: function () {
		cf.restartWorkflowCRUD.theLoadingMask = new Ext.LoadMask(Ext.getBody(), {msg:'<?php echo __('Preparing Data...',null,'workflowmanagement'); ?>'});					
		cf.restartWorkflowCRUD.theLoadingMask.show();
	...
	checkComponent: function(component) {
		...
		var number = new RegExp(cf.restartWorkflowCRUD.theNumberRegEx,"m");
		var textfield = new RegExp(cf.restartWorkflowCRUD.theTextfieldRegEx,"m");
		var textarea = new RegExp(cf.restartWorkflowCRUD.theTextareaRegEx,"m");
		var file = new RegExp(cf.restartWorkflowCRUD.theFilefieldRegEx,"m");
